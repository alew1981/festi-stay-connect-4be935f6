  ...
  const { data: musicGenres, isLoading: isLoadingGenres } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tm_tbl_events")
        .select("categories, attraction_names")
        .gte("event_date", new Date().toISOString())
        .not("categories", "is", null);
      
      if (error) throw error;
      
      // Extract unique genres from categories
      ...
      data?.forEach(event => {
        if (event.categories) {
          ...
          subgenres.forEach((subgenre: any) => {
            const name = subgenre.name || subgenre;
            if (name) {
              if (!genreMap.has(name)) {
                genreMap.set(name, { count: 1, artists: new Set() });
              } else {
              ...
              if (event.attraction_names && Array.isArray(event.attraction_names)) {
                event.attraction_names.forEach((artist: string) => {
                  genreMap.get(name).artists.add(artist);
                });
              }
            }
          });
      ...
      return Array.from(genreMap.entries())
        .map(([name, stats]) => ({
          ...
          artist_count: stats.artists.size,
                  ...
                  <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group h-full">
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                        <Badge className="bg-[#00FF8F] text-[#121212] hover:bg-[#00FF8F] border-0 font-medium uppercase">
